...
import { decryptSessionBundle, HodProService } from "../src/hodpro-service.mjs";
...
function harness() {
  ...
  const api = {
    ...
    async poll(userId, token) {
      ...
      return { maintenance_tools: [TOOL_ID] };
    }
  };
  const auth = {
    async initialize() {},
...
test("abrir aloca e busca a sessão atual antes de entregar somente o resultado operacional", async () => {
  ...
  assert.deepEqual(calls.map((entry) => entry[0]), ["list", "allocate", "session"]);
